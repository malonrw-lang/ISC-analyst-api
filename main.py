...
async def analyze(ticker: str, window: int = 12, mode: str = "edgar"):
    ...
    result = {
        ...
        'traditional': {
            ...
            'roic':            {'val': round((roic or 0)*100,2) if roic is not None else None, 'label': 'ROIC %', 'simple': 'Return on invested capital. NOPAT \u00f7 (debt + equity). NOPAT computed using {}% {} effective tax rate. Less distorted by buybacks than ROE. Above 15% is excellent.'.format(round(effective_tax_rate * 100, 1), 'firm-specific' if effective_tax_rate_source == 'computed_from_filings' else 'default'), 'effective_tax_rate': round(effective_tax_rate * 100, 1), 'effective_tax_rate_source': effective_tax_rate_source},
